...
architecture rtl of gridLockFifo is
    ...
    constant FIFO_DEPTH :integer := 2**(ADDR_WIDTH);--8192
...
wrDone       <= hi when (fifoWrAddrs    = FIFO_DEPTH - 2) else lo;
...
preFull      <= preStatus and eqlLocations;
    ...
    begin
        rdConditionBit0 := wrAddress(ADDR_WIDTH-2) xor  rdAddress(ADDR_WIDTH-1);
        rdConditionBit1 := wrAddress(ADDR_WIDTH-1) xnor rdAddress(ADDR_WIDTH-2);
    ...
    begin
        wrConditionBit0 := wrAddress(ADDR_WIDTH-2) xnor rdAddress(ADDR_WIDTH-1);
        wrConditionBit1 := wrAddress(ADDR_WIDTH-1) xor  rdAddress(ADDR_WIDTH-2);
